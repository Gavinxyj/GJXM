...
function alterArea(id)
{
    ...
    // 设置请求类型,请求地址，以及是否启用异步处理请求，大多数设置开启 true  
    xmlhttp.open("GET","area.php?active=alterArea&name="+id,true);  
    // 将请求发送至服务器  
    xmlhttp.send();  
    // 处理onreadystatechange事件 我们规定当服务器响应已做好被处理的准备时所执行的任务  
    xmlhttp.onreadystatechange=function()  
    {  
        ...
        {  
			...
			for(var index = 0; index < selectProvince.options.length; index ++)
			{
				if(selectProvince.options[index].innerHTML == "江苏省")
				{
					selectProvince.options[index].selected = true;					
					break;
				}
			...
			document.getElementById("city").innerHTML = "<option value = '扬州市'>扬州市</option>";
			document.getElementById("area").innerHTML = "<option value = " +json[0].F_NAME+ ">"+json[0].F_NAME+"</option>"  ;
		//	document.getElementById("city").innerHTML == "<option value = '扬州市'>扬州市</option>";
