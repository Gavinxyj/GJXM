...
function request(id)
{
    ...
    {  
        ...
        {  
        	...
        	document.getElementById("id").value = json.F_ID;
        	document.getElementById("name").value=json.F_FULLNAME;
        	document.getElementById("userName").value=json.F_USERNAME;
        	document.getElementById("pwd").value=json.F_PHONE;
        	document.getElementById("verifyPwd").value=json.F_PHONE;
        	document.getElementById("tel").value=json.F_PHONE;
        	
        	var context = "";
        	
        	var rank = parseInt(json.F_RANK);
        	var head = parseInt(json.F_HEAD);
        	for(var index = 0; index < json.rank.length; index ++)
    		{
        		if(rank == parseInt(json.rank[index].F_ID))
        		{
        			if(json.rank[index].F_TITLE == "管理员")
        			{
        				$("#boss").show();
        			}
        			else
        			...
        			context +="<option value = " +json.rank[index].F_ID+" selected>"+json.rank[index].F_TITLE+"</option>"
        		}
        		else
        		{
        			context +="<option value = " +json.rank[index].F_ID+">"+json.rank[index].F_TITLE+"</option>"
        		}
    		}
        	
        	document.getElementById("ad").innerHTML = context;
        	
        	var contextBoss = "";
        	for(var index = 0; index < json.boss.length; index ++)
    		{
        		if(head == parseInt(json.boss[index].F_ID))
        		{
        			contextBoss +="<option value = " +json.boss[index].F_ID+ " selected>"+json.boss[index].F_FULLNAME+"</option>"
        		}
        		else
        		{
        			contextBoss +="<option value = " +json.boss[index].F_ID+ ">"+json.boss[index].F_FULLNAME+"</option>"
        		}
    		}
        	document.getElementById("leader").innerHTML = contextBoss;
        	
        	document.getElementById("title").innerHTML = "员工信息修改";
			document.getElementById("ok").value = "确认修改";
        }  
    } 
    
   
}
function showLeader(operator)
{
	if(operator == "管理员")
	{
		$("#boss").show();
...
function addStaff(type)
{
    ...
    // 设置请求类型,请求地址，以及是否启用异步处理请求，大多数设置开启 true  
    xmlhttp.open("GET","ajax.php?active=addStaff",true);  
    // 将请求发送至服务器  
    xmlhttp.send();  
    // 处理onreadystatechange事件 我们规定当服务器响应已做好被处理的准备时所执行的任务  
    xmlhttp.onreadystatechange=function()  
	...
	if(type == "管理员" || type == "系统管理员")
	{
		document.getElementById("type").value = "manager";
	}
	else if(type == "操作人员")
	{
		document.getElementById("type").value = "operator";
	}
	else if(type == "安装人员")
	{
		document.getElementById("type").value = "install";
	}
	else if(type == "安全人员")
	{
		document.getElementById("type").value = "secutity";
	}
	...
	document.getElementById("title").innerHTML = "新增员工信息";
	document.getElementById("ok").value = "确认新增";		
}

function alterRank(id)
{
    ...
    // 设置请求类型,请求地址，以及是否启用异步处理请求，大多数设置开启 true  
    xmlhttp.open("GET","rank.php?active=alterRank&id="+id,true);  
    // 将请求发送至服务器  
    xmlhttp.send();  
    // 处理onreadystatechange事件 我们规定当服务器响应已做好被处理的准备时所执行的任务  
    xmlhttp.onreadystatechange=function()  
    {  
        ...
        {  
			...
			document.getElementById("name").value = json[0].F_TITLE;
			document.getElementById("desc").innerHTML = json[0].F_DESC;
        }  
    } 
}
function alterSecurity(id)
{
	var xmlhttp;  
    // 创建XMLHttpRequest对象  
    if (window.XMLHttpRequest)  
    ...
    // 设置请求类型,请求地址，以及是否启用异步处理请求，大多数设置开启 true  
    xmlhttp.open("GET","head.php?active=alter&id="+id,true);  
    // 将请求发送至服务器  
    xmlhttp.send();  
    // 处理onreadystatechange事件 我们规定当服务器响应已做好被处理的准备时所执行的任务  
    xmlhttp.onreadystatechange=function()  
    {  
        ...
        {  
        	var json = eval("(" + xmlhttp.responseText + ")");
        	document.getElementById("id").value = json[0].F_ID;
        	document.getElementById("name").value=json[0].F_FULLNAME;
        	document.getElementById("tel").value=json[0].F_PHONE; 
			document.getElementById("time").value=json[0].F_TIME; 
			var context = "";
			if(parseInt(json[0].F_BOSS) == 0)
			{
				context = "<option value = 0 selected>安全员</option>";
				context += "<option value = 1>主任</option>";
			}
			else
			{
				context = "<option value = 0>安全员</option>";
				context += "<option value = 1 selected>主任</option>";
			}
...
{	
	...
	for(var index = 0; index < selectProvince.options.length; index ++)
	{
		if(selectProvince.options[index].innerHTML == "--请选择省份--")
		{
	...
	document.getElementById("title").innerHTML = "新增区域信息";
	document.getElementById("ok").value = "确认新增";
}

function alterArea(id)
{
    ...
    // 设置请求类型,请求地址，以及是否启用异步处理请求，大多数设置开启 true  
    xmlhttp.open("GET","area.php?active=alterArea&id="+id,true);  
    // 将请求发送至服务器  
    xmlhttp.send();  
    // 处理onreadystatechange事件 我们规定当服务器响应已做好被处理的准备时所执行的任务  
    xmlhttp.onreadystatechange=function()  
    {  
        ...
        {  
			...
			for(var index = 0; index < selectProvince.options.length; index ++)
			{
				if(selectProvince.options[index].innerHTML == json[0].F_PROVINCE)
				{
					selectProvince.options[index].selected = true;					
					break;
				}
			...
			document.getElementById("city").innerHTML = "<option value = " +json[0].F_CITY+ ">"+json[0].F_CITY+"</option>";
			document.getElementById("area").innerHTML = "<option value = " +json[0].F_AREA+ ">"+json[0].F_AREA+"</option>";
			document.getElementById("address").value  = json[0].F_NAME;
			document.getElementById("id").value = id;
...
function addBox()
{
	document.getElementById("wh").style = "height : 400px;width : 500px";
//	document.getElementById("wh").style = "height : 400px";
	$("#rankSegment").hide();
	$("#addSegment").show();
	var xmlhttp;  
    // 创建XMLHttpRequest对象  
    ...
    // 设置请求类型,请求地址，以及是否启用异步处理请求，大多数设置开启 true  
    xmlhttp.open("GET","box.php?active=addBox",true);  
    // 将请求发送至服务器  
    xmlhttp.send();  
    // 处理onreadystatechange事件 我们规定当服务器响应已做好被处理的准备时所执行的任务  
    xmlhttp.onreadystatechange=function()  
    {  
        ...
        {  
    		...
    		{        		       			        		
        		context +="<option value = " +json.area[index].F_AREA+">"+json.area[index].F_AREA+"</option>";       	
    		}
        	
        	document.getElementById("area").innerHTML = context;
        	
			context = "";
        	for(var index = 0; index < json.road.length; index ++)
    		{        		
        		context +="<option value = " +json.road[index].F_NAME+ ">"+json.road[index].F_NAME+"</option>";        		
    		}
        	document.getElementById("road").innerHTML = context;
			
			context = "";
			for(var index = 0; index < json.user.length; index ++)
    		{        		
        		context +="<option value = " +json.user[index].F_FULLNAME+ ">"+json.user[index].F_FULLNAME+"</option>";        		
    		}
			
        	document.getElementById("user").innerHTML = context;
			
			context = "";				
			for(var index =0; index < json.version.length; index ++)
			{
				context +="<option value = " +json.version[index]+ ">"+json.version[index]+"</option>";        		
			}
			
			document.getElementById("version").innerHTML = context;
			...
			document.getElementById("preName").style = "width : 40px";
			document.getElementById("version").style = "width : 80px";
        }
    }  
}
...
function deleteByCode(code)
{
	...
	{		
		// 设置请求类型,请求地址，以及是否启用异步处理请求，大多数设置开启 true  
		xmlhttp.open("GET","box.php?active=delete&code="+code,true);  
		// 将请求发送至服务器  
		xmlhttp.send();  
		// 处理onreadystatechange事件 我们规定当服务器响应已做好被处理的准备时所执行的任务  
		xmlhttp.onreadystatechange=function()  
...
function deleteUser(userName)
{
	...
	if(confirm("你确定要删用户"+userName+"？"))
	{		
		// 设置请求类型,请求地址，以及是否启用异步处理请求，大多数设置开启 true  
		xmlhttp.open("GET","users.php?active=delete&userName="+userName,true);  
		// 将请求发送至服务器  
		xmlhttp.send();  
		// 处理onreadystatechange事件 我们规定当服务器响应已做好被处理的准备时所执行的任务  
		xmlhttp.onreadystatechange=function()  
...
function deleteArea(id)
{
	...
	if(confirm("你确定要删用户该记录吗？"))
	{		
		// 设置请求类型,请求地址，以及是否启用异步处理请求，大多数设置开启 true  
		xmlhttp.open("GET","area.php?active=delete&id="+id,true);  
		// 将请求发送至服务器  
		xmlhttp.send();  
		// 处理onreadystatechange事件 我们规定当服务器响应已做好被处理的准备时所执行的任务  
		xmlhttp.onreadystatechange=function()  
		{  
			...
			{  
				...
				if(bRet == true)
				{
					...
					var tbody = document.getElementById("areaList");
...
function alterBox(code)
{
    ...
    // 设置请求类型,请求地址，以及是否启用异步处理请求，大多数设置开启 true  
    xmlhttp.open("GET","box.php?active=alterBox&code="+code,true);  
    // 将请求发送至服务器  
    xmlhttp.send();  
    // 处理onreadystatechange事件 我们规定当服务器响应已做好被处理的准备时所执行的任务  
    xmlhttp.onreadystatechange=function()  
    {  
        ...
        {  
        	...
        	for(var index = 0; index < json.area.length; index ++)
    		{    
				if(json.box[0].F_AREA == json.area[index].F_AREA)
				{
					context +="<option value = " +json.area[index].F_AREA+" selected>"+json.area[index].F_AREA+"</option>"  
				}
				else
				{
					context +="<option value = " +json.area[index].F_AREA+">"+json.area[index].F_AREA+"</option>";  
        	...
        	for(var index = 0; index < json.road.length; index ++)
    		{       
				if(json.box[0].F_ROAD == json.road[index].F_NAME)
				{
					context +="<option value = " +json.road[index].F_NAME+ " selected>"+json.road[index].F_NAME+"</option>";
				}
				else
				{
					context +="<option value = " +json.road[index].F_NAME+ ">"+json.road[index].F_NAME+"</option>";
			...
			for(var index = 0; index < json.user.length; index ++)
    		{        
				if(json.box[0].F_USER == json.user[index].F_FULLNAME)
				{
					context +="<option value = " +json.user[index].F_FULLNAME+ " selected>"+json.user[index].F_FULLNAME+"</option>";
				}
				else
				{
					context +="<option value = " +json.user[index].F_FULLNAME+ ">"+json.user[index].F_FULLNAME+"</option>";
			...
			document.getElementById("code").value = json.box[0].F_CODE;
			document.getElementById("address").value = json.box[0].F_ADDRESS;
			document.getElementById("name").value = json.box[0].F_NAME;
...
function loadInfo()
{
    ...
    // 设置请求类型,请求地址，以及是否启用异步处理请求，大多数设置开启 true  
    xmlhttp.open("GET","box.php?active=loadInfo",true);  
    // 将请求发送至服务器  
    xmlhttp.send();  
    // 处理onreadystatechange事件 我们规定当服务器响应已做好被处理的准备时所执行的任务  
    xmlhttp.onreadystatechange=function()  
    {  
        ...
        {  
        	...
        	document.getElementById("selectRoad").innerHTML = context;
		...
		//	$("#code").hide();			
			$("#address").show();		
			$("#areaSelect").show();
			document.getElementById("id").value = "more";
        }
    } 
}
...
{
	document.getElementById("wh").style = "width : 400px";
	document.getElementById("wh").style = "height : 300px";
	var xmlhttp;  
    // 创建XMLHttpRequest对象  
    if (window.XMLHttpRequest)  
    {// code for IE7+, Firefox, Chrome, Opera, Safari 现代浏览器  
    ...
    {  
        ...
        {  
        	...
        	document.getElementById("userSegment").innerHTML = context;
			...
			document.getElementById("areaSegment").innerHTML = context;
		...
		//	$("#code").hide();	
			...
			$("#address").show();		
			$("#areaSelect").show();
			document.getElementById("id").value = "rankMulti";
        }
    } 
}
function sigleChange(codeId)
{
	var xmlhttp;  
    // 创建XMLHttpRequest对象  
    if (window.XMLHttpRequest)  
	...
	$("#areaSelect").hide();
	$("#road").hide();
	document.getElementById("codeValue").value = codeId;
	document.getElementById("id").value = "sigle";
}
...
function fileUpLoad()
{
    ...
    {  
		...
		//	alert(json.row.length);
			
			document.getElementById('fileToUpload').value = "";
			
			var tb = document.getElementById("rank");
		    
			
			for(var index = 0; index < json.row.length; index ++)
			{
				...
				row.setAttribute('id', json.row[index][0]);
				col1.innerText = json.row[index][0];
				
				var col2 = row.insertCell();				
				col2.innerText = json.row[index][1];
				
				var col3 = row.insertCell();
				col3.innerText = json.row[index][2];
				
				var col4 = row.insertCell();
				col4.innerText = json.row[index][3];
				
				var col5 = row.insertCell();
				col5.innerText = json.row[index][4];
				...
				col10.innerText = json.row[index][5];
				
				var contextAlter = "<button type = 'button' class='pui-btn pui-btn-primary theme-login' onclick = alterBox('" +json.row[index][0] +"')>修改</button>";
				
				var contextDelete = "<button type = 'button' class='pui-btn pui-btn-error' onclick = deleteByCode('" +json.row[index][0] +"')>删除</button>";
			
				var colOperator = row.insertCell();
				
				colOperator.innerHTML = contextAlter + contextDelete;
			}
			location.reload();
        }
    } 
...
function AddRank(type)
{
	...
	document.getElementById("title").value = "新增权限"
	document.getElementById("ok").value = "确认新增";
